...
function HomePage({ deals, openDeal, setRoute }) {
  ...
  return (
    <>
      ...
      <div style={{display:'grid',gridTemplateColumns:'minmax(0, 1.6fr) minmax(0, 1fr)',gap:14,padding:'0 32px 28px',flex:1,overflow:'auto'}}>
        <div className="card">
          <div className="card-h">
...
function App() {
  ...
  // Auto-collapse: only collapse the ADGA rail on small viewports; sidebar stays visible
  React.useEffect(() => {
    if (typeof window === 'undefined') return;
    if (window.innerWidth <= 1100) {
      setTweaks(prev => ({ ...prev, voiceCollapsed: true }));
    }
  }, []);
