...
import { runScheduledTick } from "@/lib/server/scheduler";

export const dynamic = "force-dynamic";

// Internal endpoint hit by scheduled() (and only by scheduled()).
// Auth: shared-secret header injected by the worker's scheduled handler.
// SECURITY: never accept user input here; only drain the queue.
export async function POST(request: Request) {
  ...
  try {
    const result = await runScheduledTick(context.env);
