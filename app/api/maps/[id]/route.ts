...
import { deleteDealFlow, getDealFlow, getDealFlowEdges, getDealFlowNodes, updateDealFlow } from "@/lib/server/repository";
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const dealFlow = await getDealFlow(auth.context.env.DB, id, DEFAULT_ORG_ID);
  ...
  const [mapPayload, hydratedNodes, hydratedEdges] = await Promise.all([
    readJsonPayload<Record<string, unknown>>(auth.context.env, dealFlow.payload_r2_key),
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await getDealFlow(auth.context.env.DB, id, DEFAULT_ORG_ID);
  ...
  const stored = auth.context.env.DB
    ...
    : null;
  const updated = await updateDealFlow(auth.context.env.DB, id, DEFAULT_ORG_ID, {
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  const removed = await deleteDealFlow(auth.context.env.DB, id, DEFAULT_ORG_ID);
  if (!removed) return errorJson("DealFlow not found.", 404);
  return json({ ok: true });
}
