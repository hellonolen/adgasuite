...
| 3 | AdgaSuite.tsx (8000+ lines) still contains 20+ workspace renderers inline. WorkspaceContract pattern shipped (`app/suite/workspaces.ts`) — KnowledgeWorkspace extracted as proof point. Next 20 workspaces need mechanical migration into `components/suite/workspaces/<Name>.tsx`, each consuming `useSuite()` from `components/suite/suite-context.tsx`. | ~$30K/mo (velocity tax, bug surface) | No | Platform | **BACKGROUND — extract opportunistically, never as the headline task.** Policy: when touching a workspace for any other reason (bug fix, copy change, agent wiring), extract it as part of that change. Never block a launch/revenue task to extract. CONTRACT_WORKSPACE_RENDERERS is the only call site that needs updating per extraction; the inline renderer stays as fallback until removed. 4 of 21 extracted (Knowledge, VoiceNotes, Messaging, Reports). Remaining 17: Home, Pending, Inbox, Tasks, Calendar, Teams, Leads, Pipeline, Story, CRM, Documents, Intelligence, Admin, Affiliates, Invoicing, Billing, Settings. |
