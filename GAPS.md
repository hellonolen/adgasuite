...
| 6 | Event-driven UI hooks exist (`lib/events/hooks.ts` + `BroadcastChannel` client mirror), but only `suite.route_viewed` is wired. Domain events emitted by agents (lead.qualified, deal.stage_changed, etc.) don't yet stream into the client bus — workspaces can't react in real time. | ~$10K/mo (perceived AI velocity) | No | Intelligence + Platform | Partial (rail laid, agents not yet emitting) |
...
| 10 | MCP server (`mcp-server.ts`) inventory exists but doesn't yet boot as a transport (stdio / HTTP). External tools can't discover capabilities yet. | Indirect; blocks orchestration from external tools | No | Conductor | Inventory shipped, transport pending |
